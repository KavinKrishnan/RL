...
"""Parameter-equality verification for the ``model_express`` refit path.
...
``model_express`` has only the end-to-end logprob metrics, which conflate refit
...
MX verifies digests on the wire, but that proves the receiver read the bytes the
publisher advertised. It cannot see a publisher that advertised the wrong slice,
which is the failure mode resharding introduces. This check reads the installed
parameters instead, so it is independent of everything MX asserts about itself.
