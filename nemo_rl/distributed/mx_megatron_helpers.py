...
def map_local_layers_to_global(
    ...
    Publishing the local index makes both PP stages of a 48-layer model offer
    ``decoder.layers.0..23``. The shard table is keyed by name and tiebroken
    first-writer-wins, so layers 24-47 become unaddressable and are never
    refit, while a stage-1 offer that wins the tiebreak fills destination layer
    N from global layer N+24 - at a valid address, with a self-consistent
    digest, undetectably. That is Bug 8, and Bug 7 is its visible remainder.
    ...
    assumptions: it handles virtual pipeline (where a rank owns several
    non-contiguous chunks) and uneven stages (``num_layers_in_first_pipeline_stage``)
    for free, both of which break the tempting ``pp_rank * layers_per_stage``.
    """
    mapping: dict[str, int] = {}
    for module_path, module in model.named_modules():
        number = getattr(module, "layer_number", None)
        # `layer_number` is 1-based and global. Require the path to end in a
        # ModuleList index so we only pick up transformer layers themselves and
        # not submodules that happen to carry the attribute for other reasons.
    ...
    if pp_size > 1 and not mapping:
        # Refusing here is deliberate. The failure this guards against produces a
        # refit that silently covers half the model and passes every correctness
        # gate we have, because those gates compare arrived bytes against the
        # publisher's digest for the same name and a name collision makes both
        # sides agree on the wrong tensor.
        raise RuntimeError(
