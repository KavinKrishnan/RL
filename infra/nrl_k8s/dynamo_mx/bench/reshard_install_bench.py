...
# MX main replaced build_installer(name, ...) with a single MdlLoader(model) whose
# load_weights callback picks the path from MX_LOAD_MODE, so the two arms this
# bench compares are now one object under two settings: "pwal" is the stock vLLM
# loader (MX_LOAD_MODE=stock) and "mdl" is the direct-write map (direct).
...
def _build(worker, name):
    ...
    return MdlLoader(worker.model_runner.model)


def w_install_timed(worker, name, n):
    import torch

    if name not in _LOAD_MODE:
        raise ValueError(f"unknown install arm {name!r}; expected one of {sorted(_LOAD_MODE)}")
    os.environ["MX_LOAD_MODE"] = _LOAD_MODE[name]
    inst = _build(worker, name)
    # load_weights takes the (name, tensor) pairs the transport would deliver.
    weights = list(worker._recv.items())
    inst.load_weights(weights)  # cold/warm-up
    ...
    return {"durations_s": durs, "load_mode": _LOAD_MODE[name]}
